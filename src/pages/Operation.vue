<template>
    <div>
        ...
        <TaxForm ref="taxFormForOperation"
                 :taxType="taxType"
                 :taxFormstep="step" />
        <div v-if="taxType === 0">
            <div v-if="step === 0">
                <v-card color="gray-3"
                    ...
                    <Table :table-options="taxListOptions"
                           :items="taxList.items"
                ...
                <v-card color="gray-3"
                        class="mb-2 pl-2 pt-3">
                    <v-layout align-center>
                        <v-flex md2>
                            <h3>過去期別未繳清礦業權者</h3>
                        </v-flex>
                    </v-layout>
                    <Table :table-options="taxUnpaidListOptions"
        ...
        <div v-else>
            ...
            <div v-show="step === 1">
                <v-card color="gray-3"
                    ...
                    <Table :table-options="taxCaseOptions"
                                ...
                                <div class="print-item"
                                    ...
                                    <div class="content">
                                        ...
                                        <div class="table">
                                            ...
                                            <div class="col-4">
                                                {{item.paylimitDate.length > 0 ? `民國${item.paylimitDate.split('-')[0] - 1911}年${item.paylimitDate.split('-')[1]}月${item.paylimitDate.split('-')[2].slice(0, 2)}日` : ''}}
                                            </div>
                                            <div class="col-2">地址</div>
                                            <div class="col-4">{{item.address}}</div>
                                            <div class="col-2">礦區面積</div>
                                            <div class="col-4">
                                                {{Math.floor(item.area)}}公頃
                                                {{item.area.toFixed(4).toString().split('.')[1].slice(0, 2)}}
                                                公畝{{item.area.toFixed(4).toString().split('.')[1].slice(2)}}平方公尺</div>
                                            ...
                                            <div class="desc">
                                                <p style="margin: 0;letter-spacing: 1px;">
                                                    ...
                                                    <span style="font-weight: bold">匯款者請註明礦區字號及礦業權者</span> 戶名：經濟部礦務局 帳號：
                ...
                <v-layout class="pt-5 pl-5">
                    ...
                    <v-flex md4>
                        ...
                        <v-layout>
                            ...
                            <v-flex md10>
                                <v-layout md8
                                          v-for="item in dialogData.royaltyReference"
                                          :key="item.mineral">
...
class PrintDataModel {
  ...
  public area: number
  public areaNo: string
  public fines: number
  public interest: number
  public licesenNo: string
  public mineConcessionFee: number
  public miningOwner: string
  public number: string
  public paylimitDate: string
...
export default class Operation extends Vue {
  ...
  public printData: any[] = [new PrintDataModel()];
  ...
  public async created() {
    // @TODO this.searchAPI
    const data: any = await addPrint({ taxId: ['5ea7f33f90194403e8e4db47'] })
  ...
  private async submitTaxAdd() {
    const { year } = (this.$refs.taxFormForOperation as any).taxListReq
    const { type } = (this.$refs.taxFormForOperation as any).taxListReq
    let collection
    if (this.taxType === 0) {
      collection = [...this.taxList.selected, ...this.taxUnpaidList.items]
    } else {
    ...
    await createTax(reqData).then((data: any) => {
      this.printList = data
      // this.resetAll()
    }).catch((err) => {
      window.alert(err)
  ...
  private async fetchAndPrint() {
    const data: any = await addPrint({ taxId: this.printList })
    this.printData = data.items;
    this.print();
  }
